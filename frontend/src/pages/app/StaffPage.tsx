...
function StaffModal({ initial, onSave, onClose }: ModalProps) {
  ...
  const validate = (): boolean => {
    ...
    if (!initial && !form.password)
      e.password = "Password wajib diisi untuk staff baru";
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 pt-5 pb-3 border-b border-gray-100">
          <h3 className="font-bold text-gray-900">
            {initial ? "Edit Staff" : "Tambah Staff"}
          </h3>
          <button
            onClick={onClose}
...
export default function StaffPage() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Hapus staff ini? Data bisa dipulihkan dari database."))
      return;
    setWorking(id);
    try {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Kelola akun staff restoran Anda
          </p>
        </div>
        <button
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {staff.map((s) => (
                  <tr
                    key={s.id}
                    className={`hover:bg-gray-50 ${s.deleted_at ? "opacity-50" : ""}`}
                  >
